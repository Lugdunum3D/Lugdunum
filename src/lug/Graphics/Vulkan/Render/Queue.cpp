...
void Queue::addMeshInstance(Scene::Node& node, const lug::Graphics::Renderer& renderer) {
    ...
    for (auto& primitiveSet : meshInstance->mesh->getPrimitiveSets()) {
        ...
        Pipeline::Id pipelineId = 0;
        {
            ...
            pipelineIdExtraPart.irradianceMapInfo = static_cast<uint32_t>(material->getIrradianceMap() ? 1 : 0);
            pipelineIdExtraPart.prefilteredMapInfo = static_cast<uint32_t>(material->getPrefilteredMap() ? 1 : 0);
